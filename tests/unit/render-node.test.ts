...
describe("Node renderer (62-2 pivot — eval in V8, no QuickJS)", () => {
	...
	it("full-JS: method calls + array methods + arrow fns", () => {
		expect(render("{{ items.map(i => i.n).join('-') }}", { items: [{ n: 1 }, { n: 2 }, { n: 3 }] })).toBe(
			"1-2-3",
		);
	});
	...
	it("@if / @else on a rich condition", () => {
		const src = "@if(users.filter(u => u.active).length > 0)some@else none@endif";
		expect(render(src, { users: [{ active: false }, { active: true }] })).toBe("some");
		expect(render(src, { users: [{ active: false }] })).toBe(" none");
	});

	it("@each over an array with @let and index binding", () => {
		const src = "@each((row, i) in rows)@let(n = i + 1){{ n }}:{{ row.name }};@endeach";
		expect(render(src, { rows: [{ name: "a" }, { name: "b" }] })).toBe("1:a;2:b;");
	});

	it("@each over an object (value, key)", () => {
		expect(render("@each((v, k) in obj){{ k }}={{ v }};@endeach", { obj: { a: 1, b: 2 } })).toBe(
			"a=1;b=2;",
		);
	});
	...
	it("THE killer case — a helper called INSIDE a rich expression (loop-scoped arg)", () => {
		...
		const helpers: HelperMap = new Map([["keep", (u: unknown) => (u as { id: number }).id % 2 === 0]]);
		expect(
			render("{{ users.filter(u => keep(u)).map(u => u.id).join(',') }}", {
	...
	it("a helper returning SafeString is emitted raw", () => {
		...
		const helpers: HelperMap = new Map([["icon", () => new SafeString("<svg/>")]]);
		expect(render("{{ [icon()][0] }}", {}, helpers)).toBe("<svg/>");
	});

	it("unknown identifier → typed error", () => {
		expect(() => render("{{ nope.x }}", {})).toThrowError(/E_INKER_UNKNOWN_IDENTIFIER|not defined/);
	});
	...
	it("@eval runs an expression for side effects, emitting nothing (62-8)", () => {
		...
		const ast: { nodes: InkerNodeJson[] } = JSON.parse(
			native.parseTemplateJson("@eval(rec('a'))kept@eval(rec('b'))", ["rec"], []),
		);
	...
	it("@<tagName>(jsArg) — a registered custom tag, buffer.writeRaw (Edge @svg)", () => {
		...
		expect(render("@svg('user')", {}, new Map(), tags)).toBe('<svg data-icon="user"/>');
	});
		...
		const tags: TagMap = new Map([
			[
				"upperName",
				{
					...
					seekable: false,
					compile(_parser, buffer, token) {
						buffer.outputExpression("name.toUpperCase()", token.filename, 1, true);
					},
				},
			],
		]);
		expect(render("@upperName()", { name: "ada" }, new Map(), tags)).toBe("ADA");
	});
	...
	it("an unregistered @tag at render time → typed error", () => {
		...
		expect(() => renderNodeTree(ast.nodes, {}, new Map(), { tags: new Map() })).toThrowError(
