...
use crate::ast::{
    ComponentArg, ComponentNode, EachBinding, IfCondition, InkerNode, LayoutNode, NamedSlot,
    SlotNode,
};
...
fn fold_if_branches(branches: Vec<IfBranch>, else_nodes: Option<Vec<InkerNode>>) -> InkerNode {
...
fn push_node(node: InkerNode, root_nodes: &mut Vec<InkerNode>, open_blocks: &mut [BlockFrame]) {
...
fn make_err(
    ...
    let mut e = InkerError::new(code, message).with_pos(line, column);
...
fn collect_helpers_in_node(node: &InkerNode, out: &mut Vec<HelperCallSite>) {
    match node {
        ...
        InkerNode::Section { body_nodes, .. } | InkerNode::PushTo { body_nodes, .. } => {
            for n in body_nodes {
                collect_helpers_in_node(n, out);
            }
        }
...
pub fn parse(tokens: &[Token], options: &ParseOptions) -> Result<InkerAst, InkerError> {
                        ...
                        match top {
                            ...
                            _ => {
                                return Err(make_err(
                                    ErrorCode::UnmatchedBlockEnd,
                                    format!(
                                        "@endslot with no open @slot (at line {pl}, column {pc})"
                                    ),
                                    pl,
                                    pc,
                                    template_path,
                    ...
                    ParsedBlockTag::Close {
                        ...
                        let top = match open_blocks.last() {
                            ...
                            None => {
                                ...
                                return Err(make_err(
                                    ErrorCode::UnmatchedBlockEnd,
                                    format!("@{kw} with no open block (at line {pl}, column {pc})"),
                                    pl,
                                    pc,
                                    template_path,
                                ));
...
mod tests {
    ...
    fn interp_emits_interpolation_node() {
        ...
        assert!(matches!(
            ast.nodes[1],
            InkerNode::Interpolation { escape: true, .. }
        ));
    }
    ...
    fn raw_interp_escape_false() {
        ...
        assert!(matches!(
            ast.nodes[0],
            InkerNode::Interpolation { escape: false, .. }
        ));
    }
