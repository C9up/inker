...
use crate::ast::{ComponentArg, ComponentNode, EachBinding, IfCondition, LayoutNode, PartialNode};
...
static REJECTED_DIRECTIVES: Lazy<HashSet<&'static str>> = Lazy::new(|| {
    ...
    for n in [
        "for", "endfor", "set", "raw", "endraw", "block", "endblock", "extends", "import", "from",
        "with", "as",
    ] {
        s.insert(n);
    }
...
fn validate_path_name(
    ...
    for segment in name.split('/') {
        ...
        if segment.is_empty() {
            return Err(fail_parse(
                format!("Template name cannot contain empty path segments; got '{name}'"),
                line,
                column,
                template_path,
            ));
...
fn parse_layout_or_include(
    ...
    if after_kw_space >= chars.len() {
        return Err(fail_parse(
            format!("{keyword} directive requires a quoted template name; got '{raw}'"),
            line,
            column,
            template_path,
        ));
    }
    let (name, next) = read_quoted_string(chars, after_kw_space, line, column, template_path)?;
...
fn parse_if_tag(
    ...
    let (expression, next_id) =
        parse_expression_with_helper_count(&expr_source, line, column, &options, helper_id_start)?;
    Ok((
        IfCondition {
            expression,
            source: expr_source,
...
fn token_prev_sig(chars: &[char], i: usize, prev: char) -> char {
    let is_comment = chars[i] == '/' && matches!(chars.get(i + 1).copied(), Some('/') | Some('*'));
    if is_comment {
        prev
    } else {
        'x'
...
fn collect_pattern_names(
    ...
    if first == '[' && last == ']' {
        for (sa, sb) in split_top_level(chars, a + 1, b - 1) {
            ...
            let ea =
                if eb - ea >= 3 && chars[ea] == '.' && chars[ea + 1] == '.' && chars[ea + 2] == '.'
                {
                    ea + 3
                } else {
                    ea
...
fn push_binding_name(
    ...
    if name.is_empty() || !rest_is_trivia {
        ...
        return Err(fail_invalid_expression(
            format!(
                "let destructuring binding '{}' is not a valid identifier",
                raw.trim()
            ),
            line,
            column,
    ...
    if is_prototype_pollution_key(&name) {
        return Err(fail_invalid_expression(
            format!(
                "let destructuring binding '{name}' is forbidden (prototype-pollution surface)"
            ),
            line,
            column,
            template_path,
...
fn parse_include_if_tag(
    ...
    let cond_source: String = body[..comma].iter().collect::<String>().trim().to_string();
    ...
    let (expression, next_id) =
        parse_expression_with_helper_count(&cond_source, line, column, &options, helper_id_start)?;
    ...
    let (name, next) = read_quoted_string(chars, name_pos, line, column, template_path)?;
    validate_path_name(&name, line, column, template_path)?;
    let after_name = skip_whitespace(chars, next);
    if after_name < chars.len() {
        let trailing: String = chars[next..].iter().collect();
...
fn parse_each_tag(
    ...
    let binding_src: String = body[..in_match.0]
        .iter()
        .collect::<String>()
        .trim()
        .to_string();
    ...
    let iterable_source: String = body[in_match.1..]
        .iter()
        .collect::<String>()
        .trim()
        .to_string();
...
fn parse_component_tag(
    ...
    if i < chars.len() {
        ...
        let (obj_expr, end_id) =
            parse_expression_with_helper_count(&obj_source, obj_line, obj_column, &opts, next_id)?;
        next_id = end_id;
...
pub fn parse_block_tag(
    ...
    if REJECTED_DIRECTIVES.contains(keyword.as_str()) {
        return Err(fail_unknown_directive(
            &keyword,
            line,
            column,
            template_path,
    ...
    if !KNOWN_KEYWORDS.contains(keyword.as_str()) {
        ...
        if let Some(closes_name) = keyword.strip_prefix("end") {
            if options.component_tags.contains_key(closes_name) {
                let trailing: String = chars[after_keyword..]
                    .iter()
                    .collect::<String>()
                    .trim()
                    .to_string();
        ...
        if options.custom_tags.contains(keyword.as_str()) {
            ...
            if options.custom_block_tags.contains(keyword.as_str()) && !explicit_self_close {
                return Ok((
    ...
    if keyword == "unless" {
        ...
        let args: String = chars[after_keyword..]
            .iter()
            .collect::<String>()
            .trim()
            .to_string();
    ...
    if keyword == "slot" {
        ...
        return Ok((
            ParsedBlockTag::OpenSlot { name, line, column },
            helper_id_start,
        ));
    }
    ...
    if keyword == "section" {
        ...
        return Ok((
            ParsedBlockTag::OpenSection { name, line, column },
            helper_id_start,
        ));
    }
    ...
    if keyword == "dump" || keyword == "dd" {
        let source =
            read_required_expression(&chars, after_keyword, &keyword, line, column, template_path)?;
        return Ok((
            ParsedBlockTag::Dump {
                source,
                die: keyword == "dd",
    ...
    if keyword == "assign" {
        ...
        let target: String = arg[..op_start]
            .iter()
            .collect::<String>()
            .trim()
            .to_string();
    ...
    if keyword == "inject" {
        let source =
            read_required_expression(&chars, after_keyword, "inject", line, column, template_path)?;
        return Ok((
    ...
    if keyword == "stack" {
        let source =
            read_required_expression(&chars, after_keyword, "stack", line, column, template_path)?;
        return Ok((
...
mod tests {
    ...
    fn each_with_helper_call_in_iterable() {
        ...
        let (pb, next_id) = parse_block_tag("each item in sorted(items)", 1, 1, &o, 5).unwrap();
        match pb {
            ParsedBlockTag::OpenEach { .. } => {}
            _ => panic!("expected OpenEach"),
        }
        assert_eq!(
            next_id, 6,
            "helper id assigned starting from 5, advanced to 6"
        );
    }

    #[test]
    fn destructured_binding_duplicate_rejected() {
        let e = parse_block_tag("each [k, k] in map", 1, 1, &opts(), 0).unwrap_err();
        assert_eq!(e.code, ErrorCode::InvalidExpression);
    }
    ...
    fn let_simple_binding_is_not_destructure() {
        ...
        match pb {
            ParsedBlockTag::Let {
                ...
                assert!(
                    !destructure,
                    "a bare identifier binding is not destructuring"
                );
            }
    ...
    fn let_destructure_extracts_bound_names_at_parse() {
        ...
        assert_eq!(
            let_names("let [first, second, ...rest] = items"),
            ["first", "second", "rest"]
        );
        // rename binds the local; default binds the key; object rest included.
        assert_eq!(
            let_names("let { a: b, role = 'guest', ...others } = u"),
            ["b", "role", "others"]
        );
    ...
    fn let_destructure_regex_default_with_comma_is_not_mis_split() {
        ...
        let (pb, _) =
            parse_block_tag("let { a = /}/.test(s), b } = obj", 1, 1, &opts(), 0).unwrap();
        match pb {
    ...
    fn inject_new_error_and_stack_capture_their_expression() {
        match parse_block_tag("inject { a: 1 }", 1, 1, &opts(), 0)
            .unwrap()
            .0
        {
        ...
        match parse_block_tag("newError 'boom', f, 1, 2", 1, 1, &opts(), 0)
            .unwrap()
            .0
        {
        ...
        match parse_block_tag("stack 'scripts'", 1, 1, &opts(), 0)
            .unwrap()
            .0
        {
    ...
    fn push_tags_carry_the_once_flag() {
        match parse_block_tag("pushTo 'scripts'", 1, 1, &opts(), 0)
            .unwrap()
            .0
        {
        ...
        match parse_block_tag("pushOnceTo 'scripts'", 1, 1, &opts(), 0)
            .unwrap()
            .0
        {
    ...
    fn push_close_tags_are_not_interchangeable() {
        ...
        match parse_block_tag("endpushOnceTo", 1, 1, &opts(), 0)
            .unwrap()
            .0
        {
            ParsedBlockTag::Close { closes, .. } => {
